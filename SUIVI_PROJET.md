...
- **Découverte en marge, non traitée** : `app/app/admin/rosters/actions.ts` (`changeTypeAction`/`submitRosterAction`/`adminInitRosterAction`/`viderRostersAction`) est du code mort — plus aucune route ne rend `RosterManager.tsx` (pas de `page.tsx` dans `admin/rosters/`) depuis la consolidation en pages hub. Seuls `addPlayerAction`/`removePlayerAction`/`updateRookieTypeAction` restent utilisés (par `/admin/init?tab=recrues`). Signalé à David, pas supprimé (hors scope de cette tâche).
